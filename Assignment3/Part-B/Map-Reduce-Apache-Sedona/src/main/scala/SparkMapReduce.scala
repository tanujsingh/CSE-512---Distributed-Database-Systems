...
object SparkMapReduce {
  ...
  def runMapReduce(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame =
  {
    ...
    var result = joinRdd.map(key=>(key.get(0),1))   // making a variable result to join RDD which is the resilient distributed dataset
   import spark.implicits._     // Importing the implicits file
    var tempresult = result.reduceByKey((k1,k2)=>k1+k2).sortBy(x=>x._2)
    var output = tempresult.map(mapk => (mapk._2)).toDF
